...
`DATA/ALARM` payload 必须携带 `origin_id` 与 `origin_seq`，使网关在经过中继后仍能打印原始传感节点 ID、原始数据序号、中继转发序号、温湿度、告警状态、ACK/蜂鸣器动作，用于现场判断丢包发生在 `sensor -> relay` 还是 `relay -> gateway`。
...
- 每 30 s 打印一次网络统计，至少包含 `rx_data`、`rx_alarm`、`tx_ack`、`origin_seq_gap_total`、`last_sync`、`offset` 和 `drift`。
