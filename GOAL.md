...
`DATA/ALARM` payload 必须携带 `origin_id` 与 `origin_seq`，使网关在经过中继后仍能打印原始传感节点 ID 和原始数据序号，用于现场判断丢包发生在 `sensor -> relay` 还是 `relay -> gateway`。
...
- Slot 2：传感节点重试 `HELLO`、采样并向父节点发送 `DATA/ALARM` 首发。
...
节点只能在自己的常规 slot 发送周期数据。`DATA` 与 `HEARTBEAT` 为周期性最佳努力发送，不要求 ACK；`ALARM` 使用跳到跳 ACK，传感节点等待中继 ACK，中继等待网关 ACK，未确认时在 Slot 4 最多重传 3 次。
...
2. 网关在 Slot 0 广播 `SYNC`，携带独立递增的 `sync_seq`、`gateway_time_ms` 和 TDMA 超帧参数；`sync_seq` 不复用普通 frame `seq`。
...
- 对 `ALARM` 返回跳到跳 ACK，普通 `DATA/HEARTBEAT` 不 ACK，体现“告警优先、周期数据可丢一帧”的 demo 策略。
